...
void loop()
{
  ...
  if(timer > 180)                                 // Execute if timer is less than 180
  { 
...
void determineLocation() 
{
  ...
  for (int x = 0; x < 4; x++) 
  {
    zone2[x] = new_state[x+4] - prev_state[x+4];              // Zone 2 tracks the upper echelon of the sensor values ( > 3)
    zone3[x] = new_state[x] - prev_state[x];                  // Zone 3 tracks the lower echelon of the sensor values ( < 3)
  }
...
void check_drums()
{
  for(int n = 0; n < 9; n++)
  {
     if((new_state[n] - prev_state[n] > threshold) && (new_state[n] > 0))                 //If sensor is triggered (new state - prev state > 30)
     {
       int vel = map(new_state[n], 0, 1023, 0, 127);                                  
       usbMIDI.sendNoteOn(note, vel, channel);                                            //MIDI Channel = 9, note = force value, velocity
     } else { 
